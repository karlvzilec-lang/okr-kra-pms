...
`scripts/verify.sql` is an assertion suite (RLS coverage across all 18 tables, the rollup regression guard, the matrix-scope join, the check-in scoring trigger, every Ruling fix across all three phases, the calibration finalize/publish gates, the comp-export HR-only boundary, the facilitator UI's HR-gated read RPCs, atomic session creation, and published-plan recalibration guard, and the goal/rating workflow's employee/manager column-scope guards and status-transition gates — 46 assertions) that runs against a freshly-reset database and fails loudly on any mismatch:
...
- **Frontend (done):** employee review summary page, the goal/rating creation workflow (employee editor + manager rating view), and the HR-only calibration facilitator board (Next.js + Tailwind + Supabase Auth/SSR).
...
**Gap-closure effort (in progress):** a brutal audit of this repo found that, despite the schema/RLS/calibration work above being solid, the core "set a goal, self-assess, get rated" loop had **no UI at all** — every rating in the seed data was hand-written SQL, and several other real gaps (no review-cycle management, no OKR creation, no pagination, zero automated frontend tests, no deploy pipeline, and more) existed alongside it. That's being worked through round by round, each round going through the same brief → independent-plan → cross-challenge → ruling → build → adversarial-review → live-verify cycle as every phase above:
  ...
  - **Rounds 2–9 (not started):** review-cycle management + OKR/check-in creation UI; admin UI (employee onboarding, goal cascade/alignment, matrix-manager scope grants); the comp-export frontend surface + pagination; calibration un-finalize/unpublish + tightening the blanket HR RLS grant on `calibration_session`/`calibration_band`; password reset + past-cycle visibility; CSP + the `middleware.ts`→`proxy.ts` migration; an automated frontend test framework; a generic audit-log table. Explicitly **not** in scope, by the user's own call when this effort started: real email/webhook notifications (no provider account exists to wire up), and any actual hosted deployment (this stays a local-only, GitHub-pushed project) — both are documented here rather than silently dropped.

See `BRIEF.md` / `RULING.md` (Phase 1), `BRIEF_PHASE2.md` / `RULING_PHASE2.md` (Phase 2), `BRIEF_PHASE3.md` / `RULING_PHASE3.md` (Phase 3), `BRIEF_CALIBRATION_UI.md` / `RULING_CALIBRATION_UI.md` (the calibration facilitator UI), and `BRIEF_GOAL_RATING_UI.md` / `RULING_GOAL_RATING_UI.md` (the goal/rating creation UI) for the full design rationale and the specific disagreements that were resolved during build — Phase 3 in particular has two real, independently-reasoned disagreements between the two builders (documented in the ruling) rather than the near-total convergence of the earlier phases, the calibration UI ruling documents a real correctness bug one builder found in the other's read of the existing schema, and the goal/rating UI ruling documents the INSERT-side security bypass one builder found that the other's otherwise-thorough plan had missed.
